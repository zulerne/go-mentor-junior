...
func TestGetRestaurantOrders_MissingRestaurantID(t *testing.T) {
	t.Parallel()
	...
	req := httptest.NewRequestWithContext(context.Background(), "GET", "/restaurant/orders", nil)
...
func TestGetRestaurantOrders_Empty(t *testing.T) {
	t.Parallel()
	...
	rest.EXPECT().GetOrders(mock.Anything, testRestaurantID).Return([]domain.Order{}, nil)
...
func TestGetRestaurantOrders_Success(t *testing.T) {
	t.Parallel()
	...
	rest.EXPECT().GetOrders(mock.Anything, testRestaurantID).Return([]domain.Order{
		{
			ID:           testOrderID,
